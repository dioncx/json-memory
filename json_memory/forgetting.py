...
class ForgettingCurve:
    ...
    def calculate_strength(
        ...
        stability = self._calculate_stability(
            initial_strength, reinforcement_count, decay_multiplier
        )
    ...
    def _calculate_stability(
        self, initial_strength: float, reinforcement_count: int, decay_multiplier: float
    ) -> float:
    ...
    def analyze_memory(
        ...
        current_strength = self.calculate_strength(
            initial_strength, last_reinforced, reinforcement_count, memory_type, current_time
        )

        predicted_forget_time = self.predict_forget_time(
            initial_strength, last_reinforced, reinforcement_count, memory_type, threshold=0.3
        )
    ...
    def prioritize_for_reinforcement(
        self,
            ...
            simulation.append(
                {
                    ...
                    "strength": strength,
                    "percent_remaining": (
                        strength / initial_strength * 100 if initial_strength > 0 else 0
                    ),
                }
            )
...
if __name__ == "__main__":
    ...
    simulation = curve.simulate_decay(
        initial_strength=1.0, reinforcement_count=0, memory_type="fact", days=30
    )
    ...
    memories = [
        {
            ...
            "value": "Dion",
            "memory_type": "identity",
            "last_reinforced": current_time - 86400 * 30,
        },
        {
            "path": "project.status",
            "value": "active",
    ...
    for item in prioritized:
        print(
            f"{item['path']}: strength={item['current_strength']:.3f}, priority={item['reinforcement_priority']:.3f}"
        )
