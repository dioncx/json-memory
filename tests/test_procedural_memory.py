...
class TestProceduralMemory:
    ...
    def test_find_transferable_skills(self, proc_mem):
        ...
        proc_mem.add_skill(
            "balance", "Forward momentum stabilizes lateral movement", domains=["cycling"]
        )
    ...
    def test_find_transferable_by_keywords(self, proc_mem):
        ...
        transferable = proc_mem.find_transferable_skills(
            "motorcycling", context_keywords={"momentum", "stabilizes"}
        )
        assert len(transferable) == 1
        assert transferable[0].name == "balance"
...
class TestSmartMemoryProcedural:
    ...
    def mem(self):
        ...
        try:
            memory = SmartMemory(path=temp_path, max_chars=5000, procedural=True)
            yield memory
        finally:
            if os.path.exists(temp_path):
                os.unlink(temp_path)
    ...
    def test_learn_extracts_principles(self, mem):
        ...
        result = mem.learn(
            experience="Learned that forward momentum causes balance stability", domain="cycling"
        )
    ...
    def test_transfer_finds_skills(self, mem):
        ...
        result = mem.transfer(new_situation="Need to balance a motorcycle", domain="motorcycling")
    ...
    def test_learn_without_procedural_fails(self):
        ...
        try:
            ...
            result = memory.learn(experience="Test experience", domain="test")
